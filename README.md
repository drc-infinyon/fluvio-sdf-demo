# Fluvio Stateful Data Flow Demo
This is a quick demo of `fluvio stateful data` flow with inline code.

This project is build using [Fluvio Open Source](https://github.com/infinyon/fluvio) and [Stateful Data Flows](https://github.com/infinyon/stateful-dataflows-examples).

Show the repos some love by giving it a star and contributing if you are into Rust.

## Fluvio
`Fluvio` is a new (over 5 years in building) distributed streaming system. Fluvio is written in Rust. Data transformations in Fluvio execute using web assembly also known as wasm.

## Stateful Data Flow
`Stateful Data Flow` or `sdf` is a development framework to build end to end data flows to process streams of events.

## Car processing demo
In this demo we use connect to a synthetic data source that mimics a data set of cars passing through two traffic locations with the license plate, make, model, speed. Then we apply transformations on the data including enrichment, anomaly detection, window based aggregation.
...
<p align="center">
 <img src="/car-processing/img/car-processing.jpg">
</p>
